class FinalBoss{
  ...
  boolean isVisible;
  int counter=(int)frameRate;
  int vel=6;
  Physics p1;
  FinalBoss(int[][] level, Physics p1){
    this.level=level;
    position=new PVector(level[0].length-(5*width/8),5);
    direction=1;
    isAlive=true;
    hp=50;
  ...
  void move(){
    if(position.x+direction<=0 || position.x+direction>=level[0].length)
      direction*=-1;
  ...
  void drawBullets(int currentx,Player p1){
    ...
    removeBullets();
    for(int i=0;i<bullets.length;i++)
      bullets[i].drawProjectile(currentx, new Enemies[0], p1, null, this);
  }
  ...
  boolean check_valid(int x1,int x2, int y1,int y2, int size, int currentx){
    for(int i=0;i<size;i++){
      if(y1+(i*y2/size)+currentx < level[0].length && y1+(i*y2/size)+currentx >=0 && level[x1 +(i*x2/size) ][y1+(i*y2/size)+currentx]==1)
        return true;
    }
    return false;
  }
